...
class BoardFetcher(QObject):
    board_received = Signal(bytes) # Emits raw bytes now
    error = Signal(str)
...
class WebSocketClient(QObject):
    ...
    def paint(self, uid, token, r, g, b, x, y):
        ...
        self.on_paint_event(x, y, r, g, b) # Optimistic update

    @Slot(bytes)
    def on_board_received(self, raw_bytes):
        self.board_data = bytearray(raw_bytes)
    ...
    def on_paint_event(self, x, y, r, g, b):
        if self.board_data and 0 <= x < 1000 and 0 <= y < 600:
            index = (y * 1000 + x) * 3
            self.board_data[index] = r
            self.board_data[index+1] = g
            self.board_data[index+2] = b
            self.emit_board_update()
